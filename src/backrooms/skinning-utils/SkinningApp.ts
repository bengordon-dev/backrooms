...
import { GUI, Mode } from "./SkinningGui.js";
import {
  sceneFSText,
  sceneVSText,
  floorFSText,
  ...
  keyframeVSText,
  keyframeFSText,
  highlightedKeyFrameFSText,
} from "./Shaders.js";
import { Mat4, Vec4, Vec3 } from "../../lib/TSM.js";
import { CLoader } from "./AnimationFileLoader.js";
import { RenderPass } from "../../lib/webglutils/RenderPass.js";
import { Camera } from "../../lib/webglutils/Camera.js";
import { KeyFrame } from "./Mesh.js";
...
export class SkinningAnimation extends CanvasAnimation {
  ...
  public initSkeleton(): void {
    ...
    this.skeletonRenderPass.addUniform(
      "highlightedIndex",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform1i(loc, this.getScene().meshes[0].highlightedBone);
      }
    )
  ...
  public initKeyframePreview(index=-1): void {
    // Status bar background
    const gl = this.ctx;
    let keyframeRenderPass = new RenderPass(
      gl,
      keyframeVSText,
      index === -1 ? keyframeFSText : highlightedKeyFrameFSText
    );
    let verts = new Float32Array([-1, -1, -1, 1, 1, 1, 1, -1]);
    keyframeRenderPass.setIndexBufferData(
      new Uint32Array([1, 0, 2, 2, 0, 3])
    );
    ...
    keyframeRenderPass.setDrawData(
      this.ctx.TRIANGLES,
      6,
      this.ctx.UNSIGNED_INT,
      0
    ...
    if (index === -1) {
      this.keyframeRenderPasses.push(keyframeRenderPass)
    } else {
  ...
  public selectKeyFrame(index: number) {
    if (this.selectedKeyFrame !== -1) {
      this.keyframeRenderPasses[this.selectedKeyFrame].setFragmentShader(keyframeFSText)
      this.keyframeRenderPasses[this.selectedKeyFrame].setup()
    }
    this.keyframeRenderPasses[index].setFragmentShader(highlightedKeyFrameFSText)
    this.keyframeRenderPasses[index].setup()
  ...
  public draw(): void {
    ...
    if (this.ctx2) {
      ...
      this.ctx2.beginPath();
      this.ctx2.moveTo(50, 770);
      this.ctx2.lineTo(750, 770);
      this.ctx2.stroke();
      if (this.gui.keyFrames.length > 1) {
        this.ctx2.beginPath();
        this.ctx2.moveTo(50, 790);
        this.ctx2.lineTo(50, 750);
        this.ctx2.stroke();
        for (let i = 0; i < this.gui.keyFrames.length - 1; i++) {
          this.ctx2.beginPath();
          this.ctx2.moveTo(50 + (i + 1) * 700 / (this.gui.keyFrames.length - 1), 790);
          this.ctx2.lineTo(50 + (i + 1) * 700 / (this.gui.keyFrames.length - 1), 750);
          this.ctx2.stroke();
        }
        this.ctx2.beginPath();
        this.ctx2.moveTo(50 + 700 * (this.gui.time / this.gui.getMaxTime()), 790);
        this.ctx2.lineTo(50 + 700 * (this.gui.time / this.gui.getMaxTime()), 750);
        this.ctx2.stroke();
      }
    }
    ...
    this.keyframeRenderPasses.forEach((e, i) => {
      gl.bindTexture(gl.TEXTURE_2D, this.textures[i]);
      gl.viewport(800, 800 - (i + 1) * targetTextureHeight, targetTextureWidth, targetTextureHeight);
      e.draw()
    });
  }

  public saveKeyframePreview(index = -1): void {
  ...
  private drawSceneRenderPass(x: number, y: number, width: number, height: number): void {
    const gl: WebGLRenderingContext = this.ctx;
